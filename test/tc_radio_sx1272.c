...
static void test_Radio_resetState(void **user)
{
    struct user_data *u = (struct user_data *)(*user);
    ...
    /* set REG_LR_OPMODE to lora mode and sleep mode */
    expect_value(board_select, state, true);    
    expect_value(board_write, data, 0x80U | REG_LR_OPMODE);
    expect_value(board_write, data, 0x80);
    expect_value(board_select, state, false);    
    
    /* set REG_LR_SYNCWORD */
    expect_value(board_select, state, true);    
    expect_value(board_write, data, 0x80U | REG_LR_SYNCWORD);
    expect_value(board_write, data, 0x34U);     // 0x34 for lora
    expect_value(board_select, state, false);    
    
    /* set REG_LR_IRQFLAGSMASK */
    expect_value(board_select, state, true);    
    expect_value(board_write, data, 0x80U | REG_LR_IRQFLAGSMASK);
    expect_value(board_write, data, 0xffU);
    expect_value(board_select, state, false);    
    
    /* set REG_LR_FIFOTXBASEADDR to 0 (RX is already at 0) */
    expect_value(board_select, state, true);    
    expect_value(board_write, data, 0x80U | REG_LR_FIFOTXBASEADDR);
    expect_value(board_write, data, 0x00U);
    expect_value(board_select, state, false);
    
    Radio_resetState(&u->radio);
}
...
static void test_Radio_transmit_noPayload(void **user)
{
    ...
    struct lora_radio_setting setting = {
        .freq = 0,
        .bw = BW_125,
        .sf = SF_6,
        .cr = CR_5,
...
static void test_Radio_sleep(void **user)
{
    ...
    expect_value(board_select, state, true);    
    expect_value(board_write, data, 0x80U | REG_LR_OPMODE);
    expect_value(board_write, data, 0x80U);
    expect_value(board_select, state, false);      
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_setup(test_Radio_resetState, setup_postInit),
