...
void ChannelList_init(struct lora_channel_list *self, enum lora_region_id region)
{
    ...
    case EU_863_870:
        ...
        (void)ChannelList_add(self, 868100000U);
        (void)ChannelList_add(self, 868300000U);
        (void)ChannelList_add(self, 868500000U);
...
bool ChannelList_add(struct lora_channel_list *self, uint32_t freq)
{
    ...
    if(freq > 0U){
        ...
        for(i=0U; i < sizeof(self->channels)/sizeof(*self->channels); i++){
            ...
            if(self->channels[i].freq == freq){
                ...
                if(self->channels[i].freq == 0U){

                    idle = (idle == NULL) ? &self->channels[i] : idle;
                }
            }
        }
    }
    ...
    if(!retval && (idle != NULL)){
        ...
        if(LoraRegion_validateFrequency(self->region, freq, &band)){
            ...
            self->bands[idle->band].numUnmasked++;
            self->numUnmasked++;
            self->numChannels++;
...
{
    int ci = findChannelByFrequency(self, freq);

    if(ci != -1){

        self->channels[ci].freq = 0U;        
        if(!self->channels[ci].masked){
            self->bands[self->channels[ci].band].numUnmasked--;
            self->numUnmasked--;
            self->numChannels--;
        }
        if(self->nextChannel == ci){ cycleChannel(self); }
    }
}
...
{
    return (size_t)self->numChannels;
}
...
static uint32_t calculateOnAirTime(const struct lora_channel_list *self, uint8_t payloadLen)
{
    ...
    if((ChannelList_bw(self) != BW_FSK) && (ChannelList_sf(self) != SF_FSK)){

        // for now hardcode this according to this recommendation
        bool lowDataRateOptimize = ((ChannelList_bw(self) == BW_125) && ((ChannelList_sf(self) == SF_11) || (ChannelList_sf(self) == SF_12))) ? true : false;
        bool crc = true;    // true for uplink, false for downlink
        bool header = (ChannelList_sf(self) == SF_6) ? false : true; 

        uint32_t Ts = ((1U << ChannelList_sf(self)) * 1000000U) / ChannelList_bw(self);     //symbol rate (us)
        uint32_t Tpreamble = (Ts * 12U) +  (Ts / 4U);       //preamble (us)

        uint32_t numerator = (8U * (uint32_t)payloadLen) - (4U * (uint32_t)ChannelList_sf(self)) + 28U + ( crc ? 16U : 0U ) - ( (header) ? 20U : 0U );
        uint32_t denom = 4U * ((uint32_t)ChannelList_sf(self) - ( lowDataRateOptimize ? 2U : 0U ));

        uint32_t Npayload = 8U + ((numerator / denom) + (((numerator % denom) != 0) ? 1U : 0U)) * ((uint32_t)ChannelList_cr(self) + 4U);
